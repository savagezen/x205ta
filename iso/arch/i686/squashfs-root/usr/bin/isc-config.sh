...
if test x"$echo_cflags" = x"true"; then
	...
	if test x"$libisc" = x"true"; then
		includes="$includes -D_REENTRANT  -I/usr/include  -D_GNU_SOURCE "
	fi
	echo $includes
fi
if test x"$echo_libs" = x"true"; then
	...
	if test x"$needothers" = x"true" ; then
		libs="$libs  -ldl -lcap -ljson-c -lpthread -lm -lGeoIP  -L/usr/lib -lxml2 -lz -llzma -lm -ldl"
	fi
	echo $libs
fi
